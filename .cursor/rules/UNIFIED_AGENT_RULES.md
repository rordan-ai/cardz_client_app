...
### 🔥 חובה לקריאה: הפרדת דחיפות (OTA/EAS Update) ובדיקות לפני פרסום
**אסור לפרסם עדכון שמערבב בין אנדרואיד לאיי־או־אס.** כל דחיפה חייבת להיות **מופרדת לפי פלטפורמה** כדי למנוע נזק לצד שלא התבקש.

**לפני כל דחיפה (EAS Update) חובה לבצע ולדווח:**
- **החלטת פלטפורמה**: לבחור *במפורש* אחת בלבד: **אנדרואיד בלבד** או **איי־או־אס בלבד** (אין “שניהם” בלי אישור מפורש).
- **פקודה בטוחה**: להשתמש בדגל פלטפורמה מתאים (למשל `--platform android` או `--platform ios`) ולוודא שהפקודה לא תפרסם לשתי פלטפורמות.
- **בדיקת בידוד בקוד**: לוודא שכל שינוי UI/התנהגות שמוגדר “אנדרואיד בלבד” עטוף בתנאי פלטפורמה (כמו `Platform.OS === 'android'`) ושב־iOS הערך ניטרלי (0/1/ללא שינוי).
- **סימון גרסה לזיהוי דחיפה**: אם משתמש מסתמך על סימון גרסה, לעדכן אותו **רק בפלטפורמה הרלוונטית** (למשל אנדרואיד בלבד) כדי לא לשנות iOS.
- **אישור משתמש לפני פרסום**: אם המשתמש דרש “אנדרואיד בלבד”/“iOS בלבד” — לא מפרסמים אחרת. אם יש ספק — עוצרים ושואלים.
