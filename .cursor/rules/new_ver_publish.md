...
## 🛑 החלטת שחרור v1.0.8 (24.09.2026): בנייה מלאה בלבד — לא OTA

**v1.0.8 משוחררת אך ורק כ-Native Build בפרופיל `production`. אסור לדחוף אותה כ-OTA.**

הסיבות:
1. **`runtimeVersion` קפוא על `"1.0.0"`** ב-app.json (שני הסעיפים, iOS ו-Android). המשמעות: עדכון OTA לערוץ `production` ינחת על **כל** ה-builds שבחנויות מאז שהערך הזה נקבע (v1.0.3 ומעלה) — לא רק על הגרסה האחרונה. שינויי v1.0.8 מניחים סכימת אחסון חדשה (`identity_verified`) ואסור שינחתו על גרסאות ישנות ללא בדיקה.
2. **פקודת ה-OTA שבנוהל זה (`--branch preview`) לא מגיעה ללקוחות** — builds של החנויות מאזינים לערוץ `production` בלבד (ראה סעיף 7ה).

הפקודה לשחרור v1.0.8 (לאחר אישור משתמש בכתב, `.env` על פרודקשן, ומיזוג PR #10):
```bash
