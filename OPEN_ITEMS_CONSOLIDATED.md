...
################################################################################
  2.5 נושאים חדשים לגרסה הבאה — ⚠️ לא לבצע עדיין (בקשת משתמש 09.08.2026)
################################################################################

  N1. 🔴 רגישות קריאת NFC — פתיחות שגויות (false triggers)
      תסמין: האפליקציה נפתחת ללקוחות בעת קרבה של **מכשירי NFC אחרים / כרטיסי אשראי**
      (לא תגי כארדז). חשד: לוגיקת ההאזנה הרציפה תופסת כל שדה NFC.
      לחקור: hooks/useNFC.ts (לולאת startReading / _globalNfcLocked) ·
      app/(tabs)/PunchCard.tsx (לולאת NFC ~661-718) · app/(tabs)/_layout.tsx
      (NfcDeepLinkHandler) · app/business/[code].tsx.
      כיוון: לסנן לפי nfc_string/פורמט תג כארדז לפני פתיחה/ניקוב, ולהתעלם משדות זרים.

  N2. הסרת הברקוד — פונקציונלית + UI (⚠️ בדיקת תלויות עמוקה)
      להסיר את הברקוד גם מה-UI וגם פונקציונלית. לבדוק לעומק תלויות לפני:
      libs: react-native-svg-barcode (PunchCard.tsx:12 `import { Barcode }`),
      react-barcode, react-native-svg-barcode (package.json) · שימושים ב-PunchCard
      וכל זרימה שנשענת על הברקוד (הצגה/ניקוב/שמירה). לוודא שאין שבירה נלווית.
      ⚠️ תיעוד בלבד כרגע — לא לבצע.
